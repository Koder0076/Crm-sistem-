...
/* ===== EARN BLOCKS (WITH USER CHECK) ===== */
app.get("/earn-blocks/:uid", async (req, res) => {
    const uid = Number(req.params.uid);

    try {
        const result = await pool.query(`
            SELECT eb.data, eb.id
            FROM earn_blocks eb
            LEFT JOIN earn_done ed
              ON ed.block_id = eb.id AND ed.user_id = $1
            WHERE ed.completed_at IS NULL
               OR ed.completed_at < NOW() - INTERVAL '23 hours'
            ORDER BY eb.id
        `, [uid]);
